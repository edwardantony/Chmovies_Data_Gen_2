...
const VideoGenres = () => {
  ...
  const fetchRowData = useCallback(async (offset: number = 0, reset: boolean = false) => {
    ...
    try {
      ...
      const url = new URL("http://localhost:5000/api/table/genre");
  ...
  const handleCreateRow = async (values: TData) => {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/table/genre", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleEditRow = async (values: TData) => {
    ...
    setIsTopToolbar(true);
    try {
      const response = await fetch(`http://localhost:5000/api/table/genre/${values.id}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleDelete = async () => {
    try {
      ...
      const deleteResponse = await fetch("http://localhost:5000/api/table/genre/delete", {
      ...
      setRowData((prevData) => {
        ...
        fetch("http://localhost:5000/api/table/genre/reorder", {
