...
interface TData {
  ...
  duration: string;
  releaseDate: Date;
  type: "Movie" | "TV Show" | "Web Series" | "Live"
...
const Title = () => {
  ...
  const fetchRowData = useCallback(async (offset: number = 0, reset: boolean = false) => {
    ...
    try {
      ...
      const url = new URL("http://localhost:5000/api/table/content");
  ...
  const columns: MRT_ColumnDef<TData>[] = useMemo(
    () => [
      ...
      // },
      {
        ...
        editSelectOptions: ["Movie", "TV Show", "Web Series", "Live"],
      },
      {
        accessorKey: "status",
        header: "Status",
  ...
  const handleCreateRow = async (values: TData) => {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/table/content", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleEditRow = async (values: TData) => {    
   // console.log("POST DATA", values)
    const newValidationErrors = validateModule(values);
   // console.log("POST Errors",newValidationErrors)
    if (Object.values(newValidationErrors).some((error) => error)) {
      setValidationErrors(newValidationErrors);
      return;
    }
    ...
    setIsTopToolbar(true);
    try {
      const response = await fetch(`http://localhost:5000/api/table/content/${values.id}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleDelete = async () => {
    try {
      ...
      const deleteResponse = await fetch("http://localhost:5000/api/table/content/delete", {
      ...
      setRowData((prevData) => {
        ...
        fetch("http://localhost:5000/api/table/content/reorder", {
...
const validateModule = (data: TData) => {
  return {
    ...
    type: data?.type?.trim() ? "" : "Row Description is required",
